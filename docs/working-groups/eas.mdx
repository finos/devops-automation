...
Meetings of the working group are held biweekly on Wednesdays at noon Eastern / 17:00 UK (see the [FINOS calendar to add yourself to the invite](https://calendar.finos.org)), and are open to anyone who would like to participate. We will follow the standard used in DevOps Automation SIG where every meeting and it's agenda will be announced, documented and tracked [via GitHub issues](https://github.com/finos/devops-automation/issues?q=is%3Aissue%20state%3Aopen%20label%3Aevolution-at-scale) using a standard template. We will meet on zoom using the standard DevOps Automation SIG details, which will be posted on every meeting issue posted.
